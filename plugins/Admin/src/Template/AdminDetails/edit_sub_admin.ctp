...
<style>
.prm-title {background: #888687 none repeat scroll 0 0; border: 0 solid #d0c7c8; color: #fff; float: left; padding: 2px 13px; width: 100%;}
...
<article class="content item-editor-page">
	...
	<div class="card card-block">
		...
		<div class="form-group row">
            ...
            <div class="col-sm-10">
               <?php echo $this->Form->input('email', ['required' => true, 'label' => false, 'class' => 'form-control boxed', 'placeholder' => 'Email Address', 'readonly'=>true]); ?>
            </div>
        </div>
	...
	<?php } ?>	
		...
		<div>
			...
			?>
				<div class="chk-box">
				<input class="ace checkbox_<?php echo strtolower($method->controller_name);?>" type="checkbox" name="admin_permisions[][admin_menu_id]" value="<?php echo $method->id;?>" id="<?php echo strtolower($method->menu_name).'_'.strtolower($method->controller_name);?>" data-controller="<?php echo strtolower($method->controller_name);?>" data-method="<?php echo strtolower($method->menu_name);?>" onclick="function_checked('<?php echo strtolower($method->menu_name);?>','<?php echo $method->id;?>','<?php echo strtolower($method->controller_name);?>');" <?php if(in_array($method->id,$selected_menus)){echo 'checked';}?>>
				<span class="lbl"> <?php echo $method->menu_name; ?></span>
				</div>
			<?php		
				}
			}
