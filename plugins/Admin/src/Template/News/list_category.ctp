...
<article class="content items-list-page">
   <div class="title-search-block">
      <div class="title-block">
         <div class="row">
            <div class="col-md-7">
               <h3 class="title">
                  News Categories
				<?php if( (array_key_exists('add-news-category',$session->read('permissions.'.strtolower('News')))) && $session->read('permissions.'.strtolower('News').'.'.strtolower('add-news-category'))==1 ){?>
                  <a href="<?php echo Router::url('/admin/news/add-news-category',true); ?>"  class="btn btn-primary btn-sm rounded-s">
                  Add New
                  </a>
				<?php }else{ ?>
					<a></a>
      ...
      <div class="items-search">
         <form class="form-inline" action="<?php echo Router::url('/admin/news/list-category/',true); ?>" method="get">
            <div class="input-group">
               <span>
                  <input value="<?php if($this->request->query('search') !== null): echo $this->request->query('search'); endif; ?>" type="text" name="search" class="form-control boxed rounded-s" placeholder="Search by category title..." />
                </span>
                <span class="input-group-btn">
                   <button class="btn btn-secondary rounded-s" type="submit">
                        <i class="fa fa-search"></i>
                   </button>
				   <a title="reset" class="btn btn-secondary rounded-s" href="<?php echo Router::url('/admin/news/list-category',true);?>">
      ...
      <ul class="item-list striped">
         ...
         <?php
          ...
          foreach($categoryDetails as $categoryDetail): ?>
                <li class="item table-data" id="row_id_<?php echo $categoryDetail->id;?>">
                    <div class="item-row">
                       <div class="item-col fixed item-col-check"> <label class="item-check">
                          <input type="checkbox" class="checkbox" value="<?php echo $categoryDetail->id; ?>">
                          <span></span>
                          </label> 
                       </div>
                       ...
                       <div class="item-col item-col-name">
                          ...
                          <div>&nbsp;<?php echo $categoryDetail->title; ?> </div>
                       </div>
					   <div class="item-col item-col-name">
                          <div class="item-heading">Added On</div>
                          <div>  <?php echo date('dS M Y',strtotime($categoryDetail->created)); ?> </div>
                       </div>
					   <div class="item-col item-col-status">
						  <div class="item-heading">Status</div>
						  <div data-id="status<?php echo $categoryDetail->id; ?>">  <?php if($categoryDetail->status == 'I'): echo "<b>Inactive</b>"; else: echo "Active"; endif; ?> </div>
					   </div>
                       <div class="item-col fixed item-col-actions-dropdown">
                          <div class="item-actions-dropdown active">
                             <div class="item-actions-block options">
								...
								<?php
								if( (array_key_exists('edit-news-category',$session->read('permissions.'.strtolower('News')))) && $session->read('permissions.'.strtolower('News').'.'.strtolower('edit-news-category'))==1 ){
								?>
                                   <li>
                                      <a class="edit" href="<?php echo Router::url("/admin/news/add-news-category",true).'/'.base64_encode($categoryDetail->id); ?>" title="Edit"> 
                                          <i class="fa fa-pencil"></i> 
                                      </a>
                                   </li>
								<?php
								}
								if( (array_key_exists('change-news-category-status',$session->read('permissions.'.strtolower('News')))) && $session->read('permissions.'.strtolower('News').'.'.strtolower('change-news-category-status'))==1 ){
								?>
								   <li>
									<?php if($categoryDetail->status == 'I'): ?>
										<a class="edit" href="javascript:void(0);" onclick="change_status('<?php echo $categoryDetail->id; ?>','A');" title="Click to Active">
											<i class="fa fa-lock" aria-hidden="true"></i>
										</a>
									<?php else: ?>
										<a class="edit" href="javascript:void(0);" onclick="change_status('<?php echo $categoryDetail->id; ?>','I');" title="Click to Inactive">
								...
								if( (array_key_exists('delete-news-category',$session->read('permissions.'.strtolower('News')))) && $session->read('permissions.'.strtolower('News').'.'.strtolower('delete-news-category'))==1 ){
								?>
								   <li>
									<a class="remove" href="javascript:void(0);" onclick="delete_category('<?php echo $categoryDetail->id; ?>');" title="Delete">
...
<?php
if( ((array_key_exists('change-news-category-status',$session->read('permissions.'.strtolower('News')))) && $session->read('permissions.'.strtolower('News').'.'.strtolower('change-news-category-status'))==1) && ((array_key_exists('delete-news-category',$session->read('permissions.'.strtolower('News')))) && $session->read('permissions.'.strtolower('News').'.'.strtolower('delete-news-category'))==1) ){
?>
	...
	swal({
	  ...
	  showLoaderOnConfirm: true
		...
		$.ajax({
			type: 'POST',
			dataType: 'JSON',
			url: '<?php echo Router::url("/admin/news/change-news-category-status/",true); ?>',
	...
	swal({
	  ...
	  showLoaderOnConfirm: true
		...
		$.ajax({
			type: 'POST',
			dataType: 'JSON',
			url: '<?php echo Router::url("/admin/news/delete-news-category/",true); ?>',
			data: {id: id},			
			success: function(result) {
				if(result.type == 'success'){
					setTimeout(function () {
			...
			$.ajax({
				...
				success: function(result) {
					if(result != ''){
						setTimeout(function () {
							swal({
								title: "Category(s) activated successfully",
								type: "success",
								confirmButtonText: "OK",
								},
								function(){
						...
						swal("No category(s) to mark as active", "", "warning");
					}
			   }
			});
        });
			...
			$.ajax({
				...
				success: function(result) {
					if(result != ''){
						setTimeout(function () {
							swal({
								title: "Category(s) inactivated successfully",
								type: "success",
								confirmButtonText: "OK",
								},
								function(){
						...
						swal("No category(s) to mark as inactive", "", "warning");
					}
				}
			});
        });
