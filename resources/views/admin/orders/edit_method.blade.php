			...
			<div class="card">
				<div class="card-body">
					@foreach (['danger', 'warning', 'success', 'info'] as $msg)
		                ...
		                @endif
		            @endforeach
					{!! Form::model($shipping_method, ['route' => ['admin.shipping_method.edit_method', base64_encode($shipping_method->id).'?redirect='.urlencode(Request::query('redirect'))], 'id' => 'shipping_methodAdd', 'class' => 'cmxform', 'method' => 'PUT', 'novalidate'] ) !!}
						<h4 class="card-title">{{ __('Update Delivery Category') }}</h4>
						<fieldset>
							<div class="form-group">
								<label for="title">{{ __('Title') }}<span class="text-danger">&#042;</span></label>
								{!! Form::text('title', null, array('required', 'class'=>'form-control', 'placeholder' => __('Enter Title'), 'id' => 'title', 'maxlength' => 20)) !!}
								<small style="float: right;color:#606060;">Characters left: <span id="shipping_title_count"><?php echo 20 - strlen($shipping_method->title);?></span></small>
								@if ($errors->has('title'))
									<span class="error">
										{{ $errors->first('title') }}
									</span>
