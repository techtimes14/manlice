...
<article class="content item-editor-page">
   ...
   <?php echo $this->Form->create($existing_property,['id' => 'login-form', 'novalidate' => 'novalidate', 'enctype'=>'multipart/form-data']); ?>
      <div class="card card-block">
		 ...
		 <div class="form-group row">
            ...
            <div class="col-sm-10">
				...
				<p class="sm_txt_nm">* Please upload jpg, png, jpeg files only, for best view please add above 60px x 60px image.</p>
				<?php
				$image = '';
				if($existing_property->image !=''){
					if (file_exists(WWW_ROOT.'uploads/property/'.$existing_property->image)){
						$image = Router::url('/uploads/property/thumb/', true).$existing_property->image;
					}else{
				...
				?>
				<span id="pro_pic">
					<img src="<?php echo $image; ?>" width='60' height='60' style="border:1px solid #000; padding:5px;" />
				</span>
         ...
         <div class="form-group row">
            <div class="col-sm-10 col-sm-offset-2">
               <?php echo $this->Form->button('Update',['type' => 'submit','class' => 'btn btn-primary']); ?>&nbsp;
				<a href="<?php echo Router::url('/admin/properties/list-data',true); ?>" class="btn btn-primary">Cancel</a>
            </div>
         </div>
      </div>
				...
				if(uploaded_file_extension=='png' || uploaded_file_extension=='jpg' || uploaded_file_extension=='jpeg' || uploaded_file_extension=='JPEG'){
					...
					image.onload = function () {
						...
						if (width < 60 && height < 60) {
							$('.error_msg').html('Image size must be above or equals to 60px x 60px, your image size is: '+this.width + "px x " + this.height+'px').css({'color':'#ff4444'});
							$('#image').val('');
							return false;
						}else{
							span.innerHTML = 
							[
							'<img class="hide_shown_image preview_image" style="width: 100px; height: 100px; border: 1px solid #000; margin: 5px 5px 5px 0; padding: 5px" src="', 
							e.target.result,
